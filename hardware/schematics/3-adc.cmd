# Pin name action command file

# Start of element CONN2
ChangePinName(CONN2, 3, 3)
ChangePinName(CONN2, 1, 1)
ChangePinName(CONN2, 2, 2)

# Start of element P2
ChangePinName(P2, 2, IN)
ChangePinName(P2, 3, OUT)
ChangePinName(P2, 1, GND)

# Start of element C6
ChangePinName(C6, 2, -)
ChangePinName(C6, 1, +)

# Start of element C5
ChangePinName(C5, 2, -)
ChangePinName(C5, 1, +)

# Start of element C4
ChangePinName(C4, 2, -)
ChangePinName(C4, 1, +)

# Start of element R2
ChangePinName(R2, 1, 1)
ChangePinName(R2, 2, 2)

# Start of element D2
ChangePinName(D2, 2, 2)
ChangePinName(D2, 1, 1)

# Start of element D1
ChangePinName(D1, 2, 2)
ChangePinName(D1, 1, 1)

# Start of element C3
ChangePinName(C3, 2, -)
ChangePinName(C3, 1, +)

# Start of element R1
ChangePinName(R1, 1, 1)
ChangePinName(R1, 2, 2)

# Start of element C1
ChangePinName(C1, 2, -)
ChangePinName(C1, 1, +)

# Start of element C2
ChangePinName(C2, 2, -)
ChangePinName(C2, 1, +)

# Start of element P1
ChangePinName(P1, 3, 3)
ChangePinName(P1, 2, 2)
ChangePinName(P1, 1, 1)

# Start of element C12
ChangePinName(C12, 2, 2)
ChangePinName(C12, 1, 1)

# Start of element CONN1
ChangePinName(CONN1, 1, 1)
ChangePinName(CONN1, 4, 4)
ChangePinName(CONN1, 2, 2)
ChangePinName(CONN1, 3, 3)

# Start of element U4
ChangePinName(U4, 2, IN)
ChangePinName(U4, 5, 5)
ChangePinName(U4, 4, GND)
ChangePinName(U4, 6, OUT)

# Start of element C11
ChangePinName(C11, 2, 2)
ChangePinName(C11, 1, 1)

# Start of element U3
ChangePinName(U3, 4, 4)
ChangePinName(U3, 11, 11)
ChangePinName(U3, 1, 1)
ChangePinName(U3, 2, 2)
ChangePinName(U3, 3, 3)

# Start of element C7
ChangePinName(C7, 2, -)
ChangePinName(C7, 1, +)

# Start of element T1

# Start of element R4
ChangePinName(R4, 1, 1)
ChangePinName(R4, 2, 2)

# Start of element R3
ChangePinName(R3, 1, 1)
ChangePinName(R3, 2, 2)
ChangePinName(R3, 1, 1)
ChangePinName(R3, 2, 2)

# Start of element R5
ChangePinName(R5, 1, 1)
ChangePinName(R5, 2, 2)

# Start of element C8
ChangePinName(C8, 2, 2)
ChangePinName(C8, 1, 1)

# Start of element U1
ChangePinName(U1, 5, REF)
ChangePinName(U1, 7, V+)
ChangePinName(U1, 4, V-)
ChangePinName(U1, 6, OUTPUT)
ChangePinName(U1, 3, IN+)
ChangePinName(U1, 8, RG2)
ChangePinName(U1, 1, RG1)
ChangePinName(U1, 2, IN-)

# Start of element C10
ChangePinName(C10, 2, 2)
ChangePinName(C10, 1, 1)

# Start of element C9
ChangePinName(C9, 2, 2)
ChangePinName(C9, 1, 1)

# Start of element U2
ChangePinName(U2, 4, IN-)
ChangePinName(U2, 3, IN+)
ChangePinName(U2, 6, CNV)
ChangePinName(U2, 7, SDO)
ChangePinName(U2, 8, SCK)
ChangePinName(U2, 9, SDI)
ChangePinName(U2, 1, REF)
ChangePinName(U2, 5, GND)
ChangePinName(U2, 10, VIO)
ChangePinName(U2, 2, VDD)
